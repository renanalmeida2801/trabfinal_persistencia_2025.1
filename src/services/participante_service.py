...
class ParticipanteService:
    ...
    async def criar_participante(
        ...
        try:
            ...
            created_participante = await self.participante_repository.create(participante)
    ...
    async def obter_participante_por_id(self, id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if resultado:
                logger.info(f"Participante encontrado: {resultado.get('nu_inscricao', 'N/A')}")
            else:
    ...
    async def obter_participante_por_inscricao(
        ...
        try:
            ...
            resultado = await self.participante_repository.find_by_inscricao(nu_inscricao)
            
            if resultado:
                logger.info(f"Participante encontrado: {nu_inscricao}")
            else:
                logger.warning(f"Participante não encontrado para inscrição: {nu_inscricao}")
                
            return resultado
            
        except Exception as e:
            logger.error(f"Erro ao buscar participante por inscrição {nu_inscricao}: {str(e)}")
            logger.error(traceback.format_exc())
            raise
    ...
    async def listar_participantes(
        ...
        try:
            ...
            logger.info(f"Encontrados {total} participantes, retornando {len(participantes)} registros")
    ...
    async def obter_estatisticas_demograficas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            estatisticas = await self.participante_repository.get_estatisticas_demograficas()
    ...
    async def obter_distribuicao_idade(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Distribuição calculada com {len(distribuicao)} faixas etárias")
            return distribuicao
    ...
    async def obter_participantes_por_escola(
        ...
        try:
            ...
            filter_dict = {"escola_codigo": escola_codigo}
            participantes = await self.participante_repository.find_all(filter_dict=filter_dict)
            
            logger.info(f"Encontrados {len(participantes)} participantes da escola {escola_codigo}")
            return participantes
            
        except Exception as e:
            logger.error(f"Erro ao buscar participantes da escola {escola_codigo}: {str(e)}")
            logger.error(traceback.format_exc())
            raise
    ...
    async def atualizar_participante(
        ...
        try:
            ...
            resultado = await self.participante_repository.update(participante_id, update_data)
    ...
    async def excluir_participante(self, participante_id: str) -> bool:
        """
