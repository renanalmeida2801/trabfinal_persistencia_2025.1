...
class MunicipioService:
    ...
    async def criar_municipio(self, municipio_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Município criado com sucesso - ID: {created_municipio.id}, Nome: {nome_municipio}"
            )
    ...
    async def listar_municipios(
        ...
        try:
            logger.info(
                f"Listando municípios - skip: {skip}, limit: {limit}, filtros: uf_sigla={uf_sigla}, regiao={regiao}"
            )
            ...
            logger.info(
                f"Encontrados {total} municípios, retornando {len(municipios)} registros"
            )
