...
class ResultadoService:
    ...
    async def criar_resultado(self, resultado_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Criando novo resultado para participante: {resultado_data.get('nu_inscricao', 'N/A')}")
            ...
            if all(
                ...
                logger.info(f"Média calculada: {resultado_data['media_provas_objetivas']}")
    ...
    async def obter_resultado_por_id(self, id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if resultado:
                logger.info(f"Resultado encontrado: {resultado.get('nu_inscricao', 'N/A')}")
            else:
    ...
    async def obter_resultado_por_participante(
        ...
        """
        try:
            logger.info(f"Buscando resultado por participante: {participante_inscricao}")
            resultado = await self.resultado_repository.find_by_participante(participante_inscricao)
            
            if resultado:
                logger.info(f"Resultado encontrado para participante: {participante_inscricao}")
            else:
                logger.warning(f"Resultado não encontrado para participante: {participante_inscricao}")
                
            return resultado
            
        except Exception as e:
            logger.error(f"Erro ao buscar resultado por participante {participante_inscricao}: {str(e)}")
            logger.error(traceback.format_exc())
            raise
    ...
    async def listar_resultados(
        ...
        """
        try:
            logger.info(f"Listando resultados - skip: {skip}, limit: {limit}, filtros: ano={ano}, escola_codigo={escola_codigo}, uf_prova_sigla={uf_prova_sigla}")
            ...
            logger.info(f"Encontrados {total} resultados, retornando {len(resultados)} registros")
