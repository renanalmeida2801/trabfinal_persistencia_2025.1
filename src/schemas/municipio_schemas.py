...
class MunicipioDetalhado(BaseModel):
    ...
    densidade_demografica: Optional[float] = Field(
        None, description="Densidade demográfica"
    )
...
class MunicipioPaginadoResponse(BaseModel):
    ...
    filtros_aplicados: Optional[Dict[str, Any]] = Field(
        None, description="Filtros aplicados"
    )
...
class EstatisticaRegiao(BaseModel):
    ...
    populacao_total: Optional[int] = Field(
        None, description="População total da região"
    )
    populacao_media: Optional[float] = Field(
        None, description="População média por município"
    )
    pib_per_capita_medio: Optional[float] = Field(
        None, description="PIB per capita médio"
    )
...
class ResumoGeralMunicipios(BaseModel):
    ...
    regiao_com_mais_municipios: Optional[str] = Field(
        None, description="Região predominante"
    )
...
class EstatisticasRegiaoResponse(BaseModel):
    ...
    estatisticas_por_regiao: List[EstatisticaRegiao] = Field(
        ..., description="Estatísticas por região"
    )
