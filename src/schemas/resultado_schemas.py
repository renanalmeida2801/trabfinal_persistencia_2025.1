...
class RankingUFItem(BaseModel):
    ...
    media_geral_objetivas: float = Field(..., description="Média geral das provas objetivas")
...
class ResumoRankingUF(BaseModel):
    ...
    maior_participacao: Optional[UFInfo] = Field(None, description="UF com maior participação")
    total_participantes: int = Field(..., description="Total de participantes no ranking")
...
class MediaPorArea(BaseModel):
    ...
    percentual_participacao: float = Field(..., description="Percentual de participação")
...
class MediasGeraisResponse(BaseModel):
    ...
    medias_por_area: List[MediaPorArea] = Field(..., description="Médias detalhadas por área")
...
class ResumoDistribuicao(BaseModel):
    ...
    participantes_com_nota_valida: int = Field(..., description="Participantes com nota válida")
...
class DistribuicaoRedacaoResponse(BaseModel):
    ...
    distribuicao_por_faixas: List[DistribuicaoFaixa] = Field(..., description="Distribuição por faixas")
...
class ParticipantesDestaqueResponse(BaseModel):
    ...
    total: int = Field(..., description="Total de participantes que atendem ao critério")
    skip: int = Field(..., description="Registros pulados")
    limit: int = Field(..., description="Limite por página")
    has_more: bool = Field(..., description="Se há mais registros")
    current_page: int = Field(..., description="Página atual")
...
class ResultadoDetalhado(BaseModel):
    ...
    participante_inscricao: str = Field(..., description="Número de inscrição do participante")
    ...
    media_provas_objetivas: Optional[float] = Field(None, description="Média das provas objetivas")
    uf_prova_sigla: Optional[str] = Field(None, description="Sigla da UF onde fez a prova")
...
class ResultadoResponse(BaseModel):
    ...
    resultado: ResultadoDetalhado = Field(..., description="Dados detalhados do resultado")
    
    
class ResultadoSimples(BaseModel):
    """Resposta simples para resultado por ID ou participante"""
    participante_inscricao: Optional[str] = Field(None, description="Número de inscrição")
    ...
    media_provas_objetivas: Optional[float] = Field(None, description="Média provas objetivas")
