...
class ParticipanteDetalhado(BaseModel):
    ...
    municipio_residencia_nome: Optional[str] = Field(None, description="Município de residência")
...
class ResumoGlobalParticipantes(BaseModel):
    ...
    percentual_treineiros: float = Field(..., description="Percentual de treineiros")
    distribuicao_anos: List[Dict[str, Any]] = Field(..., description="Distribuição por anos")
...
class EstatisticasDemograficasResponse(BaseModel):
    ...
    por_faixa_etaria: List[EstatisticaIdade] = Field(..., description="Estatísticas por idade")
    por_cor_raca: List[EstatisticaRaca] = Field(..., description="Estatísticas por cor/raça")
...
class ParticipantePorUF(BaseModel):
    ...
    densidade_participacao: Optional[str] = Field(None, description="Densidade de participação")


class ParticipantesPorUFResponse(BaseModel):
    """Resposta dos participantes por UF"""
    participantes_por_uf: List[ParticipantePorUF] = Field(..., description="Lista de participantes por UF")
    ...
    uf_com_maior_participacao: Optional[str] = Field(None, description="UF com maior participação")
...
class FaixaIdadeDistribuicao(BaseModel):
    ...
    total_participantes: int = Field(..., description="Total de participantes")
    percentual: float = Field(..., description="Percentual do total")
    idade_media_aproximada: Optional[float] = Field(None, description="Idade média aproximada")
    predominancia_sexo: Optional[str] = Field(None, description="Sexo predominante na faixa")


class DistribuicaoIdadeResponse(BaseModel):
    """Resposta da distribuição de idades"""
    distribuicao_por_faixa: List[FaixaIdadeDistribuicao] = Field(..., description="Distribuição por faixas")
    total_participantes: int = Field(..., description="Total de participantes")
    faixa_predominante: str = Field(..., description="Faixa etária predominante")
    estatisticas_gerais: Dict[str, Any] = Field(..., description="Estatísticas gerais de idade")
